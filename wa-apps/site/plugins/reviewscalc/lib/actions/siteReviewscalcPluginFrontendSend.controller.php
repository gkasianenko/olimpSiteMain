...
class siteReviewscalcPluginFrontendSendController extends waJsonController
{
    /**
     * @throws waException
     */
...
//                '<strong>Согласие на обработку персональных данных:</strong><br>'. (waRequest::request('agree_to_terms') ? 'да' : 'нет').'<br>',
                ...
                '<strong>Текст сообщения:</strong><br>' . waRequest::request('form_comment').'<br>'.waRequest::request('text').'<br>',
            ];
            ...
            try {
                ...
                if ($settings['mail_log'] == 1) {
                    if ($status) {
                        waLog::log(sprintf(_wp('Message from %s has been sent successfully'), $data['email']), 'site-reviews-calc-mail.log');
                    } else {
                        waLog::log(sprintf(_wp('Message from %s not been sended'), $data['email']), 'site-reviews-calc-mail.log');
                    }
                }
            } catch (waException $e) {
                waLog::log(_wp('Error send email from ' . $data['email']), 'site-reviews-calc-mail-error.log');
                waLog::log($e, 'site-reviews-calc-mail-error.log');
            }
        }
    }
}