...
class siteFaqPluginFrontendSendController extends waJsonController
{
    /**
     * @throws waException
     */
        ...
        if ($pass) {
            ...
            try {
                ...
                if ($settings['mail_log'] == 1) {
                    if ($status) {
                        waLog::log(sprintf(_wp('Message from %s has been sent successfully'), $data['email']), 'site-faq-mail.log');
                    } else {
                        waLog::log(sprintf(_wp('Message from %s not been sended'), $data['email']), 'site-faq-mail.log');
                    }
                }
            } catch (waException $e) {
                waLog::log(_wp('Error send email from ' . $data['email']), 'site-faq-mail-error.log');
                waLog::log($e, 'site-faq-mail-error.log');
            }
        }
    }
}